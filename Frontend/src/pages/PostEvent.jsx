...
const PostEvent = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Could not create event. Try again.')
    } finally {
      setIsLoading(false)
    }
  }
        ...
        >
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-earth-100 overflow-hidden">
            <div className="bg-gradient-to-r from-primary-600 to-secondary-600 px-6 py-8 text-white">
              <h1 className="text-2xl md:text-3xl font-display font-bold">Post a volunteer event</h1>
              <p className="mt-2 text-white/90 text-sm max-w-xl">
                Share when and where help is needed. Volunteers will discover your event on the public listings page.
              </p>
            </div>
